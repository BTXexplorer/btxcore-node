...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/btx.conf')));
var bitcoreService = proxyquire('../../lib/services/bitcored', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultbitcoreConf = fs.readFileSync(path.resolve(__dirname, '../data/default.btx.conf'), 'utf8');

describe('bitcore Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var bitcored = new bitcoreService(baseConfig);
      should.exist(bitcored);
    });
    it('will create an instance without `new`', function() {
      var bitcored = bitcoreService(baseConfig);
      should.exist(bitcored);
    });
    it('will init caches', function() {
      var bitcored = new bitcoreService(baseConfig);
      should.exist(bitcored.utxosCache);
      should.exist(bitcored.txidsCache);
      should.exist(bitcored.balanceCache);
      should.exist(bitcored.summaryCache);
      should.exist(bitcored.transactionDetailedCache);

      should.exist(bitcored.transactionCache);
      should.exist(bitcored.rawTransactionCache);
      should.exist(bitcored.blockCache);
      should.exist(bitcored.rawBlockCache);
      should.exist(bitcored.blockHeaderCache);
      should.exist(bitcored.zmqKnownTransactions);
      should.exist(bitcored.zmqKnownBlocks);
      should.exist(bitcored.lastTip);
      should.exist(bitcored.lastTipTimeout);
    });
    it('will init clients', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.nodes.should.deep.equal([]);
      bitcored.nodesIndex.should.equal(0);
      bitcored.nodes.push({client: sinon.stub()});
      should.exist(bitcored.client);
    });
    it('will set subscriptions', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: []
      });
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._initDefaults({transactionConcurrency: 10});
      bitcored.transactionConcurrency.should.equal(10);
      bitcored._initDefaults({});
      bitcored.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      bitcoreService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var bitcored = new bitcoreService(baseConfig);
      var methods = bitcored.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var bitcored = new bitcoreService(baseConfig);
      var events = bitcored.getPublishEvents();
      should.exist(events);
      events.length.should.equal(3);
      events[0].name.should.equal('bitcored/rawtransaction');
      events[0].scope.should.equal(bitcored);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('bitcored/hashblock');
      events[1].scope.should.equal(bitcored);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('bitcored/addresstxid');
      events[2].scope.should.equal(bitcored);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.subscribe = sinon.stub();
      bitcored.unsubscribe = sinon.stub();
      var events = bitcored.getPublishEvents();

      events[0].subscribe('test');
      bitcored.subscribe.args[0][0].should.equal('rawtransaction');
      bitcored.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      bitcored.unsubscribe.args[0][0].should.equal('rawtransaction');
      bitcored.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      bitcored.subscribe.args[1][0].should.equal('hashblock');
      bitcored.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      bitcored.unsubscribe.args[1][0].should.equal('hashblock');
      bitcored.unsubscribe.args[1][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var bitcored = new bitcoreService(baseConfig);
      var emitter = {};
      bitcored.subscribe('hashblock', emitter);
      bitcored.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      bitcored.subscribe('rawtransaction', emitter2);
      bitcored.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      bitcored.subscribe('hashblock', emitter1);
      bitcored.subscribe('hashblock', emitter2);
      bitcored.subscribe('hashblock', emitter3);
      bitcored.subscribe('hashblock', emitter4);
      bitcored.subscribe('hashblock', emitter5);
      bitcored.subscriptions.hashblock.length.should.equal(5);

      bitcored.unsubscribe('hashblock', emitter3);
      bitcored.subscriptions.hashblock.length.should.equal(4);
      bitcored.subscriptions.hashblock[0].should.equal(emitter1);
      bitcored.subscriptions.hashblock[1].should.equal(emitter2);
      bitcored.subscriptions.hashblock[2].should.equal(emitter4);
      bitcored.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var bitcored = new bitcoreService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      bitcored.subscriptions.hashblock= [emitter1];
      bitcored.unsubscribe('hashblock', emitter3);
      bitcored.subscriptions.hashblock.length.should.equal(1);
      bitcored.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      bitcored.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(bitcored.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var bitcored = new bitcoreService(baseConfig);
      var emitter = new EventEmitter();
      bitcored.subscribeAddress(emitter, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will handle multiple address subscribers', function() {
      var bitcored = new bitcoreService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      bitcored.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      bitcored.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
      bitcored.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      bitcored.unsubscribeAddress(emitter1);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      bitcored.unsubscribeAddress(emitter1, ['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter2];
      bitcored.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      bitcored.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      bitcored.unsubscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.not.exist(bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      bitcored.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'] = [emitter1, emitter2];
      sinon.spy(bitcored, 'unsubscribeAddressAll');
      bitcored.unsubscribeAddress(emitter1);
      bitcored.unsubscribeAddressAll.callCount.should.equal(1);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      bitcored.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      bitcored.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'] = [emitter1, emitter2];
      bitcored.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      bitcored.subscriptions.address['rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh'] = [emitter1];
      bitcored.unsubscribeAddress(emitter1);
      bitcored.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      bitcored.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'].length.should.equal(1);
      bitcored.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(bitcored.subscriptions.address['rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var bitcored = new bitcoreService(baseConfig);
      var config = bitcored._getDefaultConfig();
      config.should.equal(defaultbitcoreConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a btx.conf file', function() {
      var Testbitcore = proxyquire('../../lib/services/bitcored', {
        fs: {
          readFileSync: readFileSync,
      ...
      var bitcored = new Testbitcore(baseConfig);
      bitcored.options.spawn.datadir = '/tmp/.btx';
      var node = {};
      bitcored._loadSpawnConfiguration(node);
      should.exist(bitcored.spawn.config);
      bitcored.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: btxcore.Networks.testnet,
          configPath: '/tmp/.btxcore/btxcore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var bitcored = new Testbitcore(config);
      bitcored.options.spawn.datadir = './data';
      var node = {};
      bitcored._loadSpawnConfiguration(node);
      bitcored.options.spawn.datadir.should.equal('/tmp/.btxcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var Testbitcore = proxyquire('../../lib/services/bitcored', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badbtx.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var bitcored = new Testbitcore(baseConfig);
      (function() {
        bitcored._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(btxcore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the btx.conf file', function() {
      var bitcored = new bitcoreService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      bitcored._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        bitcored.transactionDetailedCache.set(keys[i], {});
        bitcored.utxosCache.set(keys[i], {});
        bitcored.txidsCache.set(keys[i], {});
        bitcored.balanceCache.set(keys[i], {});
        bitcored.summaryCache.set(keys[i], {});
      }
      bitcored._resetCaches();
      should.equal(bitcored.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(bitcored.utxosCache.get(keys[0]), undefined);
      should.equal(bitcored.txidsCache.get(keys[0]), undefined);
      should.equal(bitcored.balanceCache.get(keys[0]), undefined);
      should.equal(bitcored.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      bitcored._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        bitcored.nodes[0].client.getInfo.callCount.should.equal(1);
        bitcored.nodes[1].client.getInfo.callCount.should.equal(1);
        bitcored.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      bitcored._tryAllClients(function(client, next) {
        ...
        bitcored.nodes[1].client.getInfo.callCount.should.equal(1);
        bitcored.nodes[2].client.getInfo.callCount.should.equal(1);
        bitcored.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert bitcored-rpc error object into JavaScript error', function() {
      var bitcored = new bitcoreService(baseConfig);
      var error = bitcored._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      bitcored.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      bitcored.nodes.push({
        client: {
      ...
      bitcored._initChain(function() {
        log.info.callCount.should.equal(1);
        bitcored.getRawBlock.callCount.should.equal(1);
        bitcored.getRawBlock.args[0][0].should.equal('genesishash');
        bitcored.height.should.equal(5000);
        bitcored.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      bitcored._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      bitcored.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      bitcored._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      btxcore.Networks.disableRegtest();
      baseConfig.node.network = btxcore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      ...
      var bitcored = new bitcoreService(config);
      bitcored._getDefaultConf().rpcport.should.equal(8766);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var bitcored = new bitcoreService(config);
      bitcored._getDefaultConf().rpcport.should.equal(18766);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var bitcored = new bitcoreService(config);
      should.equal(bitcored._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var bitcored = new bitcoreService(config);
      bitcored._getNetworkConfigPath().should.equal('testnet3/btx.conf');
    });
    it('will get default rpc port for regtest', function() {
      btxcore.Networks.enableRegtest();
      var config = {
      ...
      var bitcored = new bitcoreService(config);
      bitcored._getNetworkConfigPath().should.equal('regtest/btx.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.node.network = btxcore.Networks.testnet;
      bitcored._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.node.network = btxcore.Networks.testnet;
      btxcore.Networks.enableRegtest();
      bitcored._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.node.network = btxcore.Networks.livenet;
      btxcore.Networks.enableRegtest();
      should.equal(bitcored._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      bitcored._rapidProtectedUpdateTip = sinon.stub();
      bitcored.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      bitcored._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      bitcored._rapidProtectedUpdateTip = sinon.stub();
      bitcored._zmqBlockHandler(node, message);
      bitcored._rapidProtectedUpdateTip.callCount.should.equal(1);
      bitcored._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      bitcored._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      bitcored._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      bitcored.subscriptions.hashblock.push(emitter);
      emitter.on('bitcored/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        bitcored._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub();
      ...
      bitcored._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      bitcored.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      bitcored.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      bitcored.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub();
      bitcored._resetCaches = sinon.stub();
      bitcored.on('tip', function(height) {
        bitcored._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        bitcored.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      bitcored.on('tip', function() {
        bitcored.syncPercentage.callCount.should.equal(0);
        done();
      });
      bitcored._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using btxcore.Transaction', function() {
      var bitcored = new bitcoreService(baseConfig);
      var wif = 'L2Gkw3kKJ6N24QcDuH4XDqt9cTqsKTVNDGz1CRZhk9cq4auDUbJy';
      var privkey = btxcore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(btxcore.Networks.testnet);
      var outputAddress = btxcore.Address('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br');
      var tx = btxcore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: btxcore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = bitcored._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(btxcore.Transaction.Output({
        script: btxcore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = bitcored._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var bitcored = new bitcoreService(baseConfig);
      var tx = btxcore.Transaction();
      var address = btxcore.Address('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br');
      tx.addOutput(btxcore.Transaction.Output({
        script: btxcore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(btxcore.Transaction.Output({
        script: btxcore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      bitcored._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      bitcored.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('bitcored/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      bitcored._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      bitcored.subscriptions.rawtransaction.push(emitter);
      emitter.on('bitcored/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      bitcored._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      bitcored.subscriptions.rawtransaction.push(emitter);
      emitter.on('bitcored/rawtransaction', function() {
        done();
      });
      var node = {};
      bitcored._zmqTransactionHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      bitcored.on('tx', function(buffer) {
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._updateTip = sinon.stub();
      bitcored._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      bitcored.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      bitcored._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        bitcored._updateTip.callCount.should.equal(11);
        bitcored._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      bitcored._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        bitcored._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      bitcored._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        bitcored._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      bitcored._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        bitcored._zmqBlockHandler.callCount.should.equal(0);
        bitcored._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var bitcored = new bitcoreService(baseConfig);
      var node = {};
      bitcored._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      bitcored._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      bitcored._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      bitcored._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      bitcored.on('tip', function(height) {
        ...
        bitcored.height.should.equal(100);
        done();
      });
      bitcored._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestbitcoreService = proxyquire('../../lib/services/bitcored', {
        fs: {
          readFile: readFile
        }
      });
      var bitcored = new TestbitcoreService(baseConfig);
      bitcored.spawnStopTime = 1;
      bitcored._process = {};
      bitcored._process.kill = sinon.stub();
      bitcored._stopSpawnedbitcore(function(err) {
        if (err) {
          return done(err);
        }
        bitcored._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      bitcored._process.kill = sinon.stub().throws(error2);
      bitcored._stopSpawnedbitcore(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._loadSpawnConfiguration = sinon.stub();
      bitcored._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      bitcored._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedbitcore', function() {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._loadSpawnConfiguration = sinon.stub();
      bitcored._stopSpawnedbitcore = sinon.stub().callsArgWith(0, new Error('test'));
      bitcored._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var bitcored = new TestbitcoreService(config);
      bitcored.spawn = {};
      bitcored._loadSpawnConfiguration = sinon.stub();
      bitcored._stopSpawnedbitcore = sinon.stub().callsArgWith(0, null);
      bitcored.node.stopping = true;
      bitcored._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      bitcored._loadSpawnConfiguration = sinon.stub();
      bitcored.spawn = {};
      bitcored.spawn.exec = 'testexec';
      bitcored.spawn.configPath = 'testdir/btx.conf';
      bitcored.spawn.datadir = 'testdir';
      bitcored.spawn.config = {};
      bitcored.spawn.config.rpcport = 20001;
      bitcored.spawn.config.rpcuser = 'bitcore';
      bitcored.spawn.config.rpcpassword = 'password';
      bitcored.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

      bitcored._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      bitcored._initZmqSubSocket = sinon.stub();
      bitcored._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      bitcored._checkReindex = sinon.stub().callsArgWith(1, null);
      bitcored._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/btx.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        bitcored._loadTipFromNode.callCount.should.equal(1);
        bitcored._initZmqSubSocket.callCount.should.equal(1);
        should.exist(bitcored._initZmqSubSocket.args[0][0].client);
        bitcored._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        bitcored._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(bitcored._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn bitcored spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestbitcoreService = proxyquire('../../lib/services/bitcored', {
        fs: {
      ...
      bitcored._loadSpawnConfiguration = sinon.stub();
      bitcored.spawn = {};
      bitcored.spawn.exec = 'bitcored';
      bitcored.spawn.datadir = '/tmp/bitcore';
      bitcored.spawn.configPath = '/tmp/bitcore/btx.conf';
      bitcored.spawn.config = {};
      bitcored.spawnRestartTime = 1;
      bitcored._loadTipFromNode = sinon.stub().callsArg(1);
      bitcored._initZmqSubSocket = sinon.stub();
      bitcored._checkReindex = sinon.stub().callsArg(1);
      bitcored._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      bitcored._stopSpawnedbitcore = sinon.stub().callsArg(0);
      sinon.spy(bitcored, '_spawnChildProcess');
      bitcored._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            bitcored._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      bitcored._spawnChildProcess(function(err) {
        ...
        bitcored._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        bitcored.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn bitcored spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestbitcoreService = proxyquire('../../lib/services/bitcored', {
        fs: {
      ...
      bitcored._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            bitcored._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      bitcored.spawn.config.rpcpassword = 'password';
      bitcored.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      bitcored._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      bitcored._spawnChildProcess(function(err) {
        bitcored._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      bitcored._initZmqSubSocket = sinon.stub();
      bitcored._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      bitcored._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var bitcored = new bitcoreService(config);
      bitcored.node.stopping = true;
      bitcored.startRetryInterval = 100;
      bitcored._loadTipFromNode = sinon.stub();
      bitcored._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        bitcored._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      bitcored.startRetryInterval = 1;
      var config = {};
      bitcored._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        bitcored._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      bitcored._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      bitcored._connectProcess(config, function(err, node) {
        should.not.exist(err);
        bitcored._loadTipFromNode.callCount.should.equal(1);
        bitcored._initZmqSubSocket.callCount.should.equal(1);
        bitcored._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      bitcored.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/bitcore configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      bitcored.options = {
        connect: [
          {}
        ]
      ...
      bitcored.start(function(err) {
        bitcored._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      var node = {};
      bitcored._initChain = sinon.stub().callsArg(0);
      bitcored._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      bitcored.options = {
        spawn: {}
      };
      bitcored.start(function(err) {
        should.not.exist(err);
        bitcored.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      bitcored._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      bitcored._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      bitcored.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    it('will give error from syncPercentage', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      bitcored.isSynced(function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      bitcored.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      bitcored.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      bitcored.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      bitcored.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      bitcored.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var bitcored = new bitcoreService(baseConfig);
      var args = bitcored._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var bitcored = new bitcoreService(baseConfig);
      var args = bitcored._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN';
      var options = {};
      bitcored.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN';
      var options = {};
      bitcored.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        bitcored.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      bitcored.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = bitcored._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = bitcored._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = bitcored._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      bitcored._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        bitcored._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      bitcored.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      bitcored.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      bitcored.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      bitcored.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        bitcored.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
          ...
          bitcored.getAddressTxids(address, {queryMempoolOnly: true}, function(err, txids) {
            if (err) {
              return done(err);
            }
            getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var bitcored = new bitcoreService(baseConfig);
      bitcored.height = 10;
      var confirmations = bitcored._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var bitcored = new bitcoreService(baseConfig);
      var result = {};
      bitcored._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var bitcored = new bitcoreService(baseConfig);
      var result = {};
      bitcored._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var bitcored = new bitcoreService(baseConfig);
      var result = {};
      bitcored._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var bitcored = new bitcoreService(baseConfig);
      var result = {};
      bitcored._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = bitcored._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      bitcored.height = 300;
      var addresses = {};
      bitcored._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      bitcored._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      bitcored._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from btxcore addresses', function() {
      var addresses = [
        btxcore.Address('RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'),
        btxcore.Address('rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh'),
      ];
      var bitcored = new bitcoreService(baseConfig);
      var strings = bitcored._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var bitcored = new bitcoreService(baseConfig);
      (function() {
        bitcored._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      bitcored.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      bitcored._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      bitcored.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var bitcored = new bitcoreService(baseConfig);
      bitcored.maxAddressesQuery = 100;
      bitcored.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      bitcored.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      bitcored.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      bitcored.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      bitcored.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      bitcored.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      bitcored.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(bitcored, '_paginateTxids');
      bitcored.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      bitcored.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      bitcored.getAddressSummary(address, options, function(err, summary) {
        bitcored._paginateTxids.callCount.should.equal(1);
        bitcored._paginateTxids.args[0][1].should.equal(0);
        bitcored._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      bitcored.getAddressSummary(address, options, function(err, summary) {
        ...
        bitcored.getAddressTxids.callCount.should.equal(1);
        bitcored.getAddressBalance.callCount.should.equal(1);
        bitcored.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          bitcored.getAddressTxids.callCount.should.equal(1);
          bitcored.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      bitcored.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      bitcored._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '3NbU8XzUgKyuCgYgZEKsBtUvkTm2r7Xgwj';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      bitcored.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      bitcored.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      bitcored.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      bitcored.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      bitcored.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as btxcore object from height', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      bitcored.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(btxcore.Block);
        done();
      });
    });
    it('will getblock as btxcore object', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      bitcored.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      bitcored.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      bitcored.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      bitcored.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      bitcored.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      bitcored.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      bitcored.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      bitcored.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      bitcored._maybeGetBlockHash('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      bitcored._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      bitcored._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      bitcored.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      bitcored.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      bitcored.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = btxcore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      bitcored.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      bitcored.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      bitcored.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = btxcore.Transaction();
      (function() {
        bitcored.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      bitcored.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      bitcored.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(btxcore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      bitcored.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      bitcored.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      bitcored.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      bitcored.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      bitcored.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      bitcored.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      bitcored.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.spawn = {};
      bitcored.spawn.process = new EventEmitter();
      bitcored.spawn.process.kill = sinon.stub();
      bitcored.stop(done);
      bitcored.spawn.process.kill.callCount.should.equal(1);
      bitcored.spawn.process.kill.args[0][0].should.equal('SIGINT');
      bitcored.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      bitcored.spawn.process.kill.callCount.should.equal(1);
      bitcored.spawn.process.kill.args[0][0].should.equal('SIGINT');
      bitcored.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var bitcored = new bitcoreService(baseConfig);
      bitcored.shutdownTimeout = 300;
      bitcored.spawn = {};
      bitcored.spawn.process = new EventEmitter();
      bitcored.spawn.process.kill = sinon.stub();
      bitcored.stop(function(err) {
