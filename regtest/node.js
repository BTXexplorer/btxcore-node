...
var btxcoreNode = index.Node;
var bitcoreService = index.services.bitcore;
...
describe('Node Functionality', function() {
  ...
  before(function(done) {
    ...
    testKey = btxcore.PrivateKey(testWIF);
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node = new btxcoreNode(configuration);

      regtest = btxcore.Networks.get('regtest');
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.bitcored.height === 150) {
            node.services.bitcored.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.bitcored.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('bitcored/hashblock', function(data) {
        bus.unsubscribe('bitcored/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.bitcored.height;
      node.services.bitcored.on('tip', function(height) {
    ...
    describe('History', function() {
      ...
      before(function(done) {
        ...
        testKey2 = btxcore.PrivateKey.fromWIF('cNfF4jXiLHQnFRsxaJyr2YSGcmtNYvxQYSakNhuDGxpkSzAwn95x');
        address2 = testKey2.toAddress(regtest).toString();

        testKey3 = btxcore.PrivateKey.fromWIF('cVTYQbaFNetiZcvxzXcVMin89uMLC43pEBMy2etgZHbPPxH5obYt');
        address3 = testKey3.toAddress(regtest).toString();

        testKey4 = btxcore.PrivateKey.fromWIF('cPNQmfE31H2oCUFqaHpfSqjDibkt7XoT2vydLJLDHNTvcddCesGw');
        address4 = testKey4.toAddress(regtest).toString();

        testKey5 = btxcore.PrivateKey.fromWIF('cVrzm9gCmnzwEVMGeCxY6xLVPdG3XWW97kwkFH3H3v722nb99QBF');
        address5 = testKey5.toAddress(regtest).toString();

        testKey6 = btxcore.PrivateKey.fromWIF('cPfMesNR2gsQEK69a6xe7qE44CZEZavgMUak5hQ74XDgsRmmGBYF');
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        var memAddress = btxcore.PrivateKey().toAddress(node.network).toString();
        var tx = new Transaction();
        tx.from(unspentOutput);
        ...
        node.services.bitcored.sendTransaction(tx.serialize(), function(err, hash) {
