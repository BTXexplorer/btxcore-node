...
var bitcoreRPC = require('bitcored-rpc');
...
/**
 * Provides a friendly event driven API to bitcored in Node.js. Manages starting and
 * stopping bitcored as a child process for application support, as well as connecting
 * to multiple bitcored processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function bitcore(options) {
  if (!(this instanceof bitcore)) {
    return new bitcore(options);
  }
...
util.inherits(bitcore, Service);

bitcore.dependencies = [];

bitcore.DEFAULT_MAX_TXIDS = 1000;
bitcore.DEFAULT_MAX_HISTORY = 50;
bitcore.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
bitcore.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
bitcore.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
bitcore.DEFAULT_SPAWN_RESTART_TIME = 5000;
bitcore.DEFAULT_SPAWN_STOP_TIME = 10000;
bitcore.DEFAULT_TRY_ALL_INTERVAL = 1000;
bitcore.DEFAULT_REINDEX_INTERVAL = 10000;
bitcore.DEFAULT_START_RETRY_INTERVAL = 5000;
bitcore.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
bitcore.DEFAULT_TRANSACTION_CONCURRENCY = 5;
bitcore.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
bitcore.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || bitcore.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || bitcore.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || bitcore.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || bitcore.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || bitcore.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || bitcore.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || bitcore.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || bitcore.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || bitcore.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || bitcore.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

bitcore.prototype._initCaches = function() {
...
bitcore.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
bitcore.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
bitcore.prototype.getPublishEvents = function() {
  return [
    {
...
bitcore.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'bitcored/' + name, 'total:', this.subscriptions[name].length);
};

bitcore.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bitcored/' + name, 'total:', this.subscriptions[name].length);
};

bitcore.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (btxcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'bitcored/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

bitcore.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'bitcored/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

bitcore.prototype.unsubscribeBalanceAll = function(emitter) {
...
bitcore.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'bitcored/addresstxid', 'total:', _.size(this.subscriptions.address));
};

bitcore.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bitcored/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
bitcore.prototype.unsubscribeAddressAll = function(emitter) {
...
bitcore.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = bitcore.DEFAULT_CONFIG_SETTINGS;
...
bitcore.prototype._parsebitcoreConf = function(configPath) {
  var options = {};
...
bitcore.prototype._expandRelativeDatadir = function() {
...
bitcore.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './btx.conf');
  ...
  _.extend(this.spawn.config, this._parsebitcoreConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parsebitcoreConf(networkConfigPath));
  }
...
bitcore.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of btxcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of btxcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of btxcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcored from btxcore. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in btx.conf'
  );
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcored is undergoing a reindex. ' +
             ...
             'of btxcore-node services will start.');
    node._reindex = true;
  }
};

bitcore.prototype._resetCaches = function() {
...
bitcore.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
bitcore.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

bitcore.prototype._initChain = function(callback) {
  var self = this;
...
bitcore.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === btxcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

bitcore.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === btxcore.Networks.testnet) {
    networkPath = 'testnet3/btx.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/btx.conf';
    }
  }
  return networkPath;
};

bitcore.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === btxcore.Networks.testnet) {
...
bitcore.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('bitcored/hashblock', message.toString('hex'));
    }
  }

};

bitcore.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
bitcore.prototype._updateTip = function(node, message) {
  var self = this;
...
bitcore.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
bitcore.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
bitcore.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('bitcored/addressbalance', {
                    address: address,
                    txid: txid,
...
bitcore.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('bitcored/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
bitcore.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = btxcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('bitcored/rawtransaction', message.toString('hex'));
    }
...
bitcore.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || bitcore.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

bitcore.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
bitcore.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
bitcore.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('bitcore Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || bitcore.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

bitcore.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
bitcore.prototype._stopSpawnedbitcore = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
bitcore.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedbitcore(function(err) {
    if (err) {
      return callback(err);
    }
...
bitcore.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
bitcore.prototype.start = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
bitcore.prototype.syncPercentage = function(callback) {
  var self = this;
...
bitcore.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, btxcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
bitcore.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = btxcore.Script.fromAddress(delta.address);
    return {
...
bitcore.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
bitcore.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
bitcore.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in btxcore is most recent to less recent
 ...
 * @param {Function} callback
 */
bitcore.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
bitcore.prototype._getConfirmationsDetail = function(transaction) {
...
bitcore.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
bitcore.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
bitcore.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A bitcore transaction id
 * @param {Function} callback
 */
bitcore.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
bitcore.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof btxcore.Address) {
...
bitcore.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
bitcore.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
bitcore.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
bitcore.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getRawBlock = function(blockArg, callback) {
...
bitcore.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a btxcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
bitcore.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = btxcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
bitcore.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
bitcore.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
bitcore.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a btxcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
bitcore.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.getInfo = function(callback) {
  var self = this;
...
bitcore.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
bitcore.prototype.stop = function(callback) {
...
bitcore.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
bitcore.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
bitcore.prototype.getMiningInfo = function(callback) {
    var self = this;
...
bitcore.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
bitcore.prototype.getPeerInfo = function(callback) {
    var self = this;
