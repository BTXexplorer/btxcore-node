...
/**
 * Will return the path and default btxcore-node configuration. It will search for the
 * configuration file in the "~/.btxcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.btxcore');
  var defaultConfigFile = path.resolve(defaultPath, './btxcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['bitcored', 'web'];
