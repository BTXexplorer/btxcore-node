...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitcored is necessary for this upgrade with the "reindex=1" btx.conf option. \n' +
                 'There are changes necessary in both btx.conf and btxcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/BTXexplorer/btxcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed btxcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './btxcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in btxcore-node services directory
    ...
    if (servicePackage.btxcoreNode) {
      serviceModule = service.name + '/' + servicePackage.btxcoreNode;
    }
 ...
 * @param {Node} node - The btxcore Node instance
 */
