...
/**
 * Will remove a service from btxcore-node.json
...
/**
 * Will remove the Node.js service and from the btxcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The btxcore-node configuration directory
...
function remove(options, done) {
  ...
  var btxcoreConfigPath = path.resolve(configPath, 'btxcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(btxcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a btxcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to btxcore-node.json
        removeConfig(btxcoreConfigPath, service, next);
      });
    }, done
  );
}
