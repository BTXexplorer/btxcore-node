...
var BASE_PACKAGE = {
  description: 'A full bitcore node build with btxcore',
  repository: 'https://github.com/BTXexplorer/btxcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a base btxcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The bitcore database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/btxcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a btxcore Node directory, configuration file,
 * bitcore configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the btxcore node configuration directory
 * @param {String} options.datadir - The path to the bitcore datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the btxcore-node directory and configuration
    ...
    function(next) {
      // Setup the bitcore directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createbitcoreDirectory(absDataDir, next);
      } else {
        next();
      }
    },
